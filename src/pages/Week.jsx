...
export default function Week({state}){
 ...
 return <div className="week-page internal-page">
  <DateNavigation leftLabel="Previous week" rightLabel="Next week" onPrevious={()=>setQueryDate(addDays(weekStart,-7))} onNext={()=>setQueryDate(addDays(weekStart,7))}>
   <h1 className="week-range">{rangeLabel(dates)}</h1><GradeFilter teachingGroups={state.teachingGroups} date={dates[0].toISOString().slice(0,10)} value={grade} onChange={setGrade}/>
  </DateNavigation>
  <section className="week-card card"><div className="week-scroll"><div className="week-grid">
   ...
   {lessonNumbers.map(number=>{
    const slot=resolveBellSlot(state.bellSchedules,dates[0].toISOString().slice(0,10),number);
    return <div className="week-row-fragment" key={number}>
     ...
     {days.map(day=>{const lesson=day.lessons.find(x=>x.number===number);if(!lesson)return <div className="week-empty-slot" key={`${weekday(day.date)}-${number}`}/>;const cls=classFor(state,lesson);return <button className={`week-lesson grade-${cls.grade||'individual'}`} style={teachingGroupColorStyle(cls)} key={lesson.id} onClick={()=>navigate(`/lesson/${lesson.id}`)}><strong>{cls.displayName}</strong><span>{shortCourseCode(lesson.code)}</span><StatusIcon type={weekStatusType(lesson,new Date())} size="small"/></button>})}
    </div>;
   })}
  </div></div></section>
 </div>;
