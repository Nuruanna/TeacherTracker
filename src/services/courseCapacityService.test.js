...
describe('per-group effective Reserve capacity',()=>{
 it('A: returns 6 for 66 slots and a 58+6 template',()=>{const value=calculateTeachingGroupCapacity(fixture(),'3B');expect(value).toMatchObject({availableScheduledSlots:66,requiredPlannedLessons:58,templateReserve:6,remainingReserve:6});});
 it('B: keeps all 6 reserves when 64 slots fit the approved map',()=>{const value=calculateTeachingGroupCapacity(fixture(),'3A');expect(value).toMatchObject({availableScheduledSlots:64,maximumEffectiveReserve:6,remainingReserve:6});});
 it('C/D: Cancel consumes and Restore returns one unit',()=>{let state=cancelCourseEvent(fixture(),'3A','event-1');expect(calculateTeachingGroupCapacity(state,'3A').remainingReserve).toBe(5);state=restoreCourseEvent(state,'3A','event-1');expect(calculateTeachingGroupCapacity(state,'3A').remainingReserve).toBe(6);});
 it('E: a Custom Lesson consumes one unit',()=>{const state=addCustomLesson(fixture(),'3A',{id:'custom-1',title:'Revision games'});expect(calculateTeachingGroupCapacity(state,'3A').remainingReserve).toBe(5);});
 it('F: a normal Reschedule does not consume reserve',()=>{const state=recordReschedule(fixture(),'3A',{eventId:'event-1',to:'2026-02-02'});expect(calculateTeachingGroupCapacity(state,'3A').remainingReserve).toBe(6);});
 it('G: warns when 2 planned lessons do not fit into 56 slots',()=>{const state=fixture();state.teachingGroups.find(item=>item.id==='3C').activeFrom='2026-03-16';const value=calculateTeachingGroupCapacity(state,'3C');expect(value).toMatchObject({availableScheduledSlots:56,missingPlannedLessons:2,hasCapacityWarning:true,remainingReserve:0});expect(capacityWarning(value).type).toBe('capacity');});
