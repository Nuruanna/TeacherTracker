...
describe('Month view',()=>{
 ...
 it('shows past instructional timetable occurrences without materializing them',()=>{const state=fresh();for(const courseState of Object.values(state.teachingGroupCourseStates))courseState.currentPosition=1;const before=JSON.stringify(state);const lessons=lessonsForDate(state,new Date(2026,7,11,12),new Date('2026-08-12T12:00:00+10:00'));expect(lessons.length).toBeGreaterThan(0);expect(lessons.every(item=>item.planned)).toBe(true);expect(JSON.stringify(state)).toBe(before);});
});
