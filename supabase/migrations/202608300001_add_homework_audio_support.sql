-- Additive Homework MP3 support. Prepared locally; apply only after review.

begin;

alter table public.homework_assets
  drop constraint if exists homework_assets_asset_type_check;

alter table public.homework_assets
  add constraint homework_assets_asset_type_check check (
    asset_type = any (array['image'::text, 'link'::text, 'pdf'::text, 'audio'::text])
  );

alter table public.homework_assets
  drop constraint if exists homework_assets_required_fields;

alter table public.homework_assets
  add constraint homework_assets_required_fields check (
    (
      asset_type = 'link'
      and url is not null
      and btrim(url) <> ''
    ...
    or
    (
      asset_type in ('image', 'pdf', 'audio')
      and bucket = 'class-site-assets'
        ...
        'template', jsonb_build_object(
          ...
          'assets', coalesce((
            ...
            where asset.owner_id = template.owner_id
              ...
              and asset.asset_type in ('image', 'audio')
        ...
        and (
          ...
          or exists (
            ...
            where meaningful_asset.homework_template_id = template.id
              ...
              and meaningful_asset.asset_type in ('image', 'audio')
